...
export type ValidationEvent = 
  | { type: 'VALIDATE_CREDENTIAL'; credential: VC }
  | { type: 'VALIDATE_BATCH'; credentials: VC[] }
  ...
  | { type: 'UPDATE_RULES'; rules: ValidationRule[] }
  | { type: 'VALIDATE_SIGNATURE'; credential: VC }
  | { type: 'VALIDATE_EXPIRY'; credential: VC }
  | { type: 'VALIDATE_ISSUER'; credential: VC }
  | { type: 'VALIDATE_SCHEMA'; credential: VC }
...
export const validationMachine = createMachine<ValidationContext, ValidationEvent>(
  {
    id: 'validation',
    initial: 'idle',
    context: {
  ...
  },
  {
    actions: {
      ...
      removeValidationRule: assign({
        validationRules: (context, event) => 
          context.validationRules.filter(rule => rule.id !== event.ruleId),
      }),
      toggleValidationRule: assign({
        validationRules: (context, event) =>
          context.validationRules.map(rule =>
            rule.id === event.ruleId ? { ...rule, enabled: event.enabled } : rule
          ),
      }),
