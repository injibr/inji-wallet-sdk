...
export class StorageService {
  ...
  async getCredential(vcId: string): Promise<VC> {
    ...
    try {
      ...
      const credential = JSON.parse(storedData) as VC;
  ...
  private async updateCredentialIndex(vcId: string, credential: VC): Promise<void> {
    try {
      ...
      await AsyncStorage.setItem(
        `vc-sdk-index-${this.config.appId}`,
  ...
  private async getCredentialIndex(): Promise<Record<string, any>> {
    try {
  ...
  private async removeFromIndex(vcId: string): Promise<void> {
    try {
      const index = await this.getCredentialIndex();
      delete index[vcId];
  ...
  private applyFilters(credentialIds: string[], index: Record<string, any>, filters: any): string[] {
    return credentialIds.filter(vcId => {
      ...
      if (filters.tags && !filters.tags.some((tag: string) => metadata.tags.includes(tag))) return false;
  ...
  async getStorageStats(): Promise<StorageStats> {
    try {
      ...
      const totalSize = JSON.stringify(index).length + 
        credentials.reduce((size, vc) => size + JSON.stringify(vc).length, 0);

      return {
        totalSize,
        credentialCount: credentials.length,
        backupCount: 0, // TODO: Implement backup counting
  ...
  async exportData(format: 'json' | 'csv'): Promise<ExportData> {
    try {
      const credentials = await this.getCredentials();
