 ...
 */
export async function generateKeyPairINJI(keyType) {
  ...
  try {
    ...
    if (!hasKey) {
      ...
      if (keyType === 'RS256') {
        await RNSecureKeystoreModule.generateKeyPairRSA(ISSUERS_KEY_REF, 2048,
        // RSA 2048 like INJI
        false,
        // requireAuth
        300,
        ...
        await RNSecureKeystoreModule.generateKeyPairEC(ISSUERS_KEY_REF, 'P-256',
        // NIST P-256 curve
        false, 300, false);
      }
 ...
 */
async function createSignatureINJI(privateKeyAlias, signingInput, keyType) {
  console.log('[INJI_JWT] Creating signature with native keystore...');
  try {
    let signature64;
    if (Platform.OS === 'android') {
      // Android: Use RNSecureKeystoreModule.sign
      signature64 = await RNSecureKeystoreModule.sign(privateKeyAlias, signingInput, true // Use base64 encoding
      );
      console.log('[INJI_JWT] ✅ Android signature created');
    } else if (Platform.OS === 'ios') {
      // iOS: Use RNSecureKeystoreModule.signWithAlias
      signature64 = await RNSecureKeystoreModule.signWithAlias(privateKeyAlias, signingInput);
    ...
    if (keyType === 'ES256') {
      const convertDerToRsFormat = derSig => {
 ...
 */
export async function getJWTInjiStyle(header, payload, keyType = 'ES256') {
  ...
  try {
    ...
    const {
      publicKey,
      privateKey
    } = await generateKeyPairINJI(keyType);
 ...
 */
async function createJWKFromPublicKey(publicKey, keyType) {
  if (keyType === 'ES256') {
 ...
 */
export async function getJWTFallback(header, payload) {
...
//# sourceMappingURL=INJICompatibleJWT.js.map