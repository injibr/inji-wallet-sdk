 ...
 */
export class ExpoNetworkAdapter implements INetworkAdapter {
  private Network: any;
   ...
   */
  private mapConnectionType(expoType: any): string {
    if (!expoType) {
      return 'unknown';
    }
  ...
  async getNetworkInfo(): Promise<any> {
    ...
    try {
      const [networkState, ipAddress] = await Promise.all([
        this.Network.getNetworkStateAsync(),
        this.Network.getIpAddressAsync().catch(() => null),
      ]);
