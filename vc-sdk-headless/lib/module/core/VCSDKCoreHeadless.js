...
import { ShareVCService } from "../services/ShareVCService.js";
import { notificationService } from "../services/NotificationService.js";
import { sdkEvents } from "../events/index.js";
export class VCSDKCoreHeadless {
  ...
  async init(config) {
    ...
    try {
      ...
      this.events.emitSDKError(error, 'initialization');
      throw error;
    }
  }
    ...
    return {
      ...
      download: async (issuer, credentialType) => {
        ...
        try {
          ...
          if (error.message === 'CREDENTIAL_NOT_AVAILABLE_424' || error.statusCode === 424) {
            this.events.emitCredentialNotAvailable424(credentialType, issuer, 'Provider does not have this credential');
            return null;
          }

          // Check for auth error
          if (error.message?.includes('401') || error.message?.includes('invalid_token') || error.message?.includes('expired') || error.message?.includes('No access token')) {
      ...
      delete: async credentialId => {
        ...
        try {
          ...
          this.events.emitSDKError(error, 'credential:delete');
          throw error;
        }
      }
    };
    ...
    return {
      ...
      getAll: async () => {
        ...
        try {
          ...
          this.events.emitIssuerError(error);
          throw error;
        }
      },
      /**
  ...
  get auth() {
    return {
       ...
       */
      completeAuthentication: async token => {
        this.ensureInitialized();
        try {
          ...
          this.events.emitAuthError(error, error.message);
          throw error;
        }
      },
      /**
    ...
    return {
      ...
      parseRequest: async shareUrl => {
        ...
        try {
          ...
          const requestedPatterns = inputDescriptors.map(descriptor => {
            const typeConstraint = descriptor.constraints?.fields?.find(f => f.path?.includes('$.type'));
            return {
          ...
          this.events.emitShareError(error, 'parse');
          throw error;
        }
      },
      /**
       ...
       */
      downloadCredentials: async requestedCredentials => {
        this.ensureInitialized();
        try {
          ...
          for (let i = 0; i < requestedCredentials.length; i++) {
            ...
            try {
              ...
              const credTypes = await this.services.issuer.getCredentialTypes(issuer.id);
              const matchingCredType = credTypes.find(ct => ct.id === credType.credentialTypeId);
              if (!matchingCredType) {
              ...
              if (error.message === 'CREDENTIAL_NOT_AVAILABLE_424' || error.statusCode === 424) {
                ...
                this.events.emitShareCredentialNotAvailable424(credType.credentialTypeId, credType.name, credType.system);
                error424Count++;
                continue;
              }

              // Check for auth error
              if (error.message?.includes('401') || error.message?.includes('invalid_token') || error.message?.includes('No access token')) {
                // Emit auth required event - host app should handle authentication
          ...
          this.events.emitShareError(error, 'download');
          throw error;
        }
      },
      /**
       ...
       */
      completeShare: async (authRequest, requestedCredentials) => {
        this.ensureInitialized();
        try {
          ...
          const credentialsToShare = requestedCredentials.map(rc => {
            const cred = allCredentials.find(c => {
              const credTypeId = c.metadata?.credentialType?.id || '';
              const vcTypes = c.type || [];
              const matchById = credTypeId === rc.type || credTypeId.includes(rc.type) || rc.type.includes(credTypeId);
              const matchByType = vcTypes.some(t => t === rc.type || t.includes(rc.type) || rc.type.includes(t));
          ...
          const matchedCreds = await this.services.shareVC.matchCredentials(authRequest.presentation_definition, credentialsToShare);
          ...
          this.events.emitShareError(error, 'sharing');
          throw error;
        }
      },
      /**
    ...
    return {
      ...
      register: async (cpf, fcmToken) => {
        ...
        try {
          ...
          this.events.emitDeviceRegistrationError(error);
          throw error;
        }
      }
    };
    ...
    return {
      ...
      handleNotification: notification => {
        ...
        if (notification.type === 'credential_ready') {
          this.events.emitNotificationCredentialReady(notification.credentialType || '', notification.issuer || '', notification.id || '');
        }
      }
    };
  }
...
//# sourceMappingURL=VCSDKCoreHeadless.js.map