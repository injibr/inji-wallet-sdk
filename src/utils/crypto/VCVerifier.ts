...
export class VCVerifier {
   ...
   */
  async verify(credential: VC): Promise<VerificationResult> {
    const errors: string[] = [];
  ...
  private validateStructure(credential: VC, errors: string[]): void {
  ...
  private async verifyProof(credential: VC, errors: string[], warnings: string[]): Promise<boolean> {
    try {
  ...
  private async verifyEd25519Signature(credential: VC, warnings: string[]): Promise<boolean> {
    try {
  ...
  private isValidIssuerFormat(issuer: string): boolean {
  ...
  private async performSemanticValidation(credential: VC, warnings: string[]): Promise<void> {
    try {
      ...
      if (subjectKeys.length <= 1) { // Only 'id' field
   ...
   */
  async verifyBatch(credentials: VC[]): Promise<VerificationResult[]> {
