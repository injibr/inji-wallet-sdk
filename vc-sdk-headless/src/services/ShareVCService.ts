...
export class ShareVCService {
  ...
  async constructVerifiablePresentation(
    ...
    try {
      ...
      Object.entries(selectedVCs).forEach(([inputDescriptorId, vcs]) => {
        vcs.forEach(vc => {
          credentialMapping.push({ inputDescriptorId, vc });
        });
      });
      ...
      if (credentialMapping.length > 0) {
        console.log('[ShareVC] Sample VC structure (first 500 chars):',
          JSON.stringify(credentialMapping[0].vc).substring(0, 500));
      }
      ...
      const wrappedVCs = credentialMapping.map(item => {
        ...
        const issuerInfo = (vcMetadata as any).issuerInfo || {};
        const credentialTypeInfo = (vcMetadata as any).credentialType || {};
        ...
        const originalCredential = (vcMetadata as any).originalCredential ||
                                   (vcMetadata as any).originalResponse?.credential;
        ...
        if (!originalCredential && vc.expirationDate) {
          (credential as any).expirationDate = vc.expirationDate;
        }
        ...
        const injiVCMetadata = {
          idType: '', // UIN or VID - not present in vc-sdk
          requestId: vc.id || '',
          isPinned: vcMetadata.isPinned || false,
          id: vc.id || '',
          issuer: issuerInfo.name || credentialTypeInfo.name || 'Unknown',  // ← Display name from well-known (e.g., "INCRA")
          protocol: 'OpenId4VCI',
          timestamp: generatedOn,  // ← Timestamp (e.g., "2025-10-07T14:02:33.556Z")
          isVerified: true,
          mosipIndividualId: '', // Not present in vc-sdk
          format: 'ldp_vc',
          isExpired: false,
        ...
        const injiVC = {
          format: 'ldp_vc',  // ← MUST BE FIRST
          generatedOn: generatedOn,  // ← SECOND
          vcMetadata: injiVCMetadata,  // ← THIRD
          verifiableCredential: {  // ← FOURTH (LAST)
            credential: credential, // ← EXACT credential from server (no modifications!)
            format: 'ldp_vc',  // ← CRITICAL: format field inside verifiableCredential!
            wellKnown: issuerInfo.wellknown_endpoint ||
                      (issuerInfo.issuerUrl ?
                        `${issuerInfo.issuerUrl.replace(/\/$/, '')}/.well-known/openid-credential-issuer`
                        : ''),
        ...
        console.log('[ShareVC] INJI VC structure created (exact format from actual request):', {
          ...
          vcMetadata: {
            issuer: injiVCMetadata.issuer,  // ← Display name ("INCRA")
            credentialType: injiVCMetadata.credentialType,
            protocol: injiVCMetadata.protocol,
            timestamp: injiVCMetadata.timestamp,
          },
          credential: {
            usingOriginal: !!originalCredential,  // ← Using exact server response
            issuer: credential.issuer,  // ← DID from server
            hasProof: !!credential.proof,
      ...
      console.log('[ShareVC] INJI VCs prepared for VP:', {
        ...
        vcStrings: stringifiedINJIVCs.map((vcStr: string) => {
          try {
      ...
      const vp: VP = {
        ...
        holder: holderDID,
        verifiableCredential: stringifiedINJIVCs as any, // INJI VCs as JSON strings
        proof: {} as any, // Will be filled by JWT
      ...
      vp.proof = {
        ...
        created: new Date().toISOString().replace(/\.\d{3}Z$/, 'Z'), // Remove milliseconds
        challenge: authRequest.nonce,  // Use nonce as challenge
        domain: 'OpenID4VP',
        jws: vpToken,
  ...
  async sendVerifiablePresentation(
    ...
    try {
      ...
      const verificationMethod = (vp.proof as any).verificationMethod;

      const vpTokenObject = {
        verifiableCredential: vp.verifiableCredential,
        id: vp.id,
        holder: '',  // Empty string as per Inji format
        proof: {
          type: vp.proof.type,
          created: vp.proof.created,
          challenge: (vp.proof as any).challenge,
          domain: (vp.proof as any).domain,
          jws: (vp.proof as any).jws,
      ...
      vpTokenObject.verifiableCredential?.forEach((cred: any, index: number) => {
        const isString = typeof cred === 'string';
        console.log(`[ShareVC] Credential ${index + 1}:`, {
          isString: isString,  // ← Should be true (INJI format)
      ...
      const credentialMapping = (vp as any)._credentialMapping || [];
