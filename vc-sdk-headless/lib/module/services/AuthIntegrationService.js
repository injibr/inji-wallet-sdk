...
export class AuthIntegrationService {
  ...
  static async isUserAuthenticated() {
    try {
      const [accessToken, idToken, expiresAt] = await Promise.all([SecureStore.getItemAsync('access_token'), SecureStore.getItemAsync('id_token'), AsyncStorage.getItem('expires_at')]);
  ...
  static async getAllAuthDataFromStorage() {
    try {
      const [accessToken, idToken, rawUserData, metaPairs] = await Promise.all([SecureStore.getItemAsync('access_token'), SecureStore.getItemAsync('id_token'), SecureStore.getItemAsync('user_data'), AsyncStorage.multiGet(['token_type', 'expires_at', 'scope'])]);
  ...
  static async getAccessToken() {
    try {
      ...
      const [accessToken, expiresAt] = await Promise.all([SecureStore.getItemAsync('access_token'), AsyncStorage.getItem('expires_at')]);
      if (!accessToken) {
  ...
  static async isSessionValid() {
    try {
      ...
      const expiresAt = authData.token.expiresAt;
  ...
  static async clearAuthData() {
    try {
      ...
      await Promise.all([SecureStore.deleteItemAsync('access_token'), SecureStore.deleteItemAsync('id_token'), SecureStore.deleteItemAsync('auth_code'), SecureStore.deleteItemAsync('c_nonce'), SecureStore.deleteItemAsync('user_data'), AsyncStorage.multiRemove(['token_type', 'expires_at', 'scope', 'auth_state'])]);
