  ...
  async constructVerifiablePresentation(selectedVCs, authRequest) {
    try {
      ...
      Object.entries(selectedVCs).forEach(([inputDescriptorId, vcs]) => {
        vcs.forEach(vc => credentialMapping.push({
          inputDescriptorId,
          vc
        }));
      });
      ...
      const rawCredentials = credentialMapping.map(({
        vc
      }) => {
        const vcMeta = vc.metadata;
      ...
      const publicKeyBase64url = Buffer.from(this.publicKey).toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '');
      const holderDID = `did:jwk:${Buffer.from(JSON.stringify({
        kty: 'OKP',
        crv: 'Ed25519',
      ...
      const [docNQuads, proofNQuads] = await Promise.all([jsonld.canonize(vpBody, canonizeOpts), jsonld.canonize(proofOptions, canonizeOpts)]);
