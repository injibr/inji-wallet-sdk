 ...
 */
export class ExpoCryptoAdapter {
  constructor() {}
  async initialize() {
    try {
  ...
  async signWithRSA(data, privateKey) {
    try {
      ...
      const jwt = await new jose.SignJWT({
        ...
        alg: 'RS256'
      }).setIssuedAt().setExpirationTime('1h').sign(privateKeyObj);
      return jwt;
    } catch (joseError) {
  ...
  async verifyRSASignature(data, signature, publicKey) {
    try {
      const jose = await this.loadJose();
      ...
      if (signature.includes('.')) {
        ...
        const {
          payload
        } = await jose.jwtVerify(signature, publicKeyObj);
  ...
  async generateJWT(payload, privateKey) {
    try {
      const jose = await this.loadJose();
      const privateKeyObj = await jose.importPKCS8(privateKey, 'RS256');
...
//# sourceMappingURL=ExpoCryptoAdapter.js.map