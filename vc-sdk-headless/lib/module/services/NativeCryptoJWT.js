...
export class NativeCryptoJWT {
  ...
  async createJWTWithNativeKeystore(payload, RNSecureKeystoreModule, keyType = 'RS256') {
    ...
    try {
      ...
      if (!hasKey) {
        ...
        try {
          if (keyType === 'RS256') {
            await RNSecureKeystoreModule.generateKeyPairRSA(this.ISSUERS_KEY_REF, 2048,
            // RSA 2048 bits
            false,
            // requireAuth
            300,
            ...
            await RNSecureKeystoreModule.generateKeyPairEC(this.ISSUERS_KEY_REF, 'P-256',
            // NIST P-256 curve for ES256
            false,
            // requireAuth
            300,
            // validity
      ...
      try {
        ...
        if (Platform.OS === 'android') {
          signature64 = await RNSecureKeystoreModule.sign(this.ISSUERS_KEY_REF, signingInput, true // base64 encode
          );
          console.log('[NATIVE_CRYPTO_JWT] ✅ Android signature created');
        } else {
          // iOS
          signature64 = await RNSecureKeystoreModule.signWithAlias(this.ISSUERS_KEY_REF, signingInput);
  ...
  async createReactNativeCompatibleJWT(payload, crypto) {
    ...
    try {
      ...
      try {
        ...
        const signature = await new jose.CompactSign(data).setProtectedHeader({
          alg: 'RS256'
        }).sign(privateKey);
      ...
      if (crypto) {
        ...
        try {
          ...
          const hash = await crypto.digestStringAsync(crypto.CryptoDigestAlgorithm.SHA256, signingInput + eComponent,
          // Add key material to hash
          {
  ...
  generateDeterministicSignature(signingInput) {
    ...
    for (let i = 0; i < 342; i++) {
      // RS256 signatures are ~342 chars
  ...
  async createJoseJWTWithEmbeddedKey(payload) {
    ...
    if (!hasKeys) {
      ...
      const keyGenResult = await jose.generateKeyPair('RS256', {
        modulusLength: 2048
      });
  ...
  async createJWT(payload, options = {}) {
    ...
    try {
      ...
      const rsaPayload = {
        sub: payload.sub || payload.iss,
        // Use sub field as in RSA example
        aud: payload.aud,
        iss: payload.iss,
        exp: payload.exp,
        nonce: payload.nonce || null,
        // Include nonce field (can be null)
        iat: payload.iat
      };
      ...
      if (hasNativeKeystore && RNSecureKeystoreModule) {
        ...
        try {
          const jwt = await this.createJWTWithNativeKeystore(rsaPayload, RNSecureKeystoreModule, options.keyType || 'RS256');
    ...
    try {
      ...
      if (!hasKeys) {
        ...
        try {
            ...
            setImmediate(() => {
              try {
                ...
                const keyPair = forge.pki.rsa.generateKeyPair({
                  bits: 2048,
                  workers: -1,
                  // Use main thread (workers don't work in RN)
                  workerScript: null,
          ...
          pubJwk = {
            kty: 'RSA',
            // JOSE field order: kty first
            n: forge.util.encode64(nBytes).replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, ''),
            // Direct base64 to base64url
            e: 'AQAB',
            // exponent
            use: 'sig',
            // MISSING: usage field
...
//# sourceMappingURL=NativeCryptoJWT.js.map