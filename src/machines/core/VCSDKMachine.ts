...
export type VCSDKMachineEvents =
  | { type: 'INIT'; config: VCSDKConfig }
  | { type: 'AUTHENTICATE' }
  | { type: 'LOGOUT' }
  | { type: 'ADD_CREDENTIAL'; credentialData: any }
  | { type: 'DELETE_CREDENTIAL'; vcId: string }
  | { type: 'LOAD_CREDENTIALS' }
  | { type: 'BACKUP_DATA' }
  | { type: 'RESTORE_DATA'; backupData: any }
  | { type: 'RESET' }
  | { type: 'ERROR'; error: string };

export const createVCSDKMachine = (services: SDKServices) => {
  return createMachine<VCSDKMachineContext, VCSDKMachineEvents>({
    id: 'vcSdk',
              ...
              onDone: {
                ...
                actions: assign({
                  credentials: (context, event) => 
                    context.credentials.filter(vc => vc.id !== event.data.vcId),
                  error: () => null,
                }),
              },
              onError: {
    ...
    services: {
      ...
      checkAuthentication: async (context) => {
        ...
        return { success: isAuthenticated, user };
      },
