...
export const issuerMachine = createMachine({
  id: 'issuer',
  initial: 'idle',
  context: {
    availableIssuers: [],
  ...
  actions: {
    ...
    searchIssuers: assign({
      ...
      filteredIssuers: (context, event) => {
        ...
        return context.availableIssuers.filter(issuer => issuer.name.toLowerCase().includes(query) || issuer.description.toLowerCase().includes(query));
      }
    }),
    searchCredentialTypes: assign({
      ...
      filteredCredentialTypes: (context, event) => {
        const query = event.query.toLowerCase();
        return context.credentialTypes.filter(type => type.name.toLowerCase().includes(query) || type.description.toLowerCase().includes(query));
      }
    }),
...
//# sourceMappingURL=IssuerMachine.js.map