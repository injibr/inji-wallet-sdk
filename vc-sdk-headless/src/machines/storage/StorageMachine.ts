...
export type StorageEvent = 
  | { type: 'LOAD_STATS' }
  | { type: 'EXPORT_DATA'; format: 'json' | 'csv' | 'pdf' }
  | { type: 'CREATE_BACKUP' }
  | { type: 'RESTORE_BACKUP'; backupData: string }
  | { type: 'CLEAR_STORAGE' }
  | { type: 'OPTIMIZE_STORAGE' }
  | { type: 'TOGGLE_ENCRYPTION'; enabled: boolean }
  | { type: 'CHECK_QUOTA' }
  | { type: 'RETRY' }
  | { type: 'RESET' };

export const storageMachine = createMachine<StorageContext, StorageEvent>(
  {
    id: 'storage',
    initial: 'idle',
    context: {
