...
export async function request(method, path, body, host = DEFAULT_BASE_URL, headers = {
  ...
  if (response.status >= 400) {
    ...
    const errorMessage = jsonResponse.message || (typeof jsonResponse.error === 'object' ? JSON.stringify(jsonResponse.error) : jsonResponse.error) || `HTTP ${response.status} ${response.statusText}`;
